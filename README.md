...
GitHub Actions로 **Ready News 기반 발행**을 구성했습니다.

- `publish-article.yml`: `main` 브랜치에 `data/ready-news/**/*.json`이 반영되면 `_posts/`로 발행
- `publish-article-*.yml`: `repository_dispatch` 또는 수동 실행으로 카테고리별 즉시 발행
...
2) 수동 최적화
```bash
python3 scripts/optimize_image.py <path/to/image.jpg>
```
...
커밋/푸시는 GitHub Actions 단계에서 수행합니다. `repository_dispatch` 경로도 카테고리별 워크플로에서 `_posts/` 변경을 같은 방식으로 저장소에 반영합니다.
...
- 카테고리별 GitHub Actions가 `client_payload.category`를 기준으로 발행
- 실제 `_posts/` 생성과 커밋/푸시는 Actions에서 처리
